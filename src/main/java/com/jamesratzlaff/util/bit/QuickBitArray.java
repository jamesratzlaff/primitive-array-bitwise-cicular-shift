...
public class QuickBitArray {
	...
	public static final int BITS_PER_UNIT = BitUnit.INT.bits();// 32
	public static final int BITS_PER_UNIT_SHIFT = BitUnit.INT.multOrDivShift();// (1<<BITS_PER_UNIT_SHIFT)==BITS_PER_UNIT;
	public static final int BIT_SHIFT_UNIT_LIMIT_MASK = BitUnit.INT.limitMask();// for ints this would be 31 (0x1F) and
																				// for longs it would be 63 (0x3F)
...
//	}
	...
	public QuickBitArray() {
		this.size = 0;
		this.bits = EMPTY_BITS;
	}

	public QuickBitArray(int size) {
		this.size = size;
		this.bits = makeArray(size);
	}

	// For testing only
	public QuickBitArray(int[] bits, int size) {
		this.bits = bits;
		this.size = size;
	}
	...
	public boolean get(int i) {
		return (bits[(i >>> BITS_PER_UNIT_SHIFT)] & (1 << (i & BIT_SHIFT_UNIT_LIMIT_MASK))) != 0;
	}
	...
	public void set(int i) {
		bits[(i >>> BITS_PER_UNIT_SHIFT)] |= 1 << (i & BIT_SHIFT_UNIT_LIMIT_MASK);
	}

	public void unset(int i) {
		bits[(i >>> BITS_PER_UNIT_SHIFT)] = 0;
	}
	...
	public int getNextSet(int from) {
		...
		currentBits &= -(1 << (from & BIT_SHIFT_UNIT_LIMIT_MASK));
		while (currentBits == 0) {
		...
		int result = (bitsOffset << BITS_PER_UNIT_SHIFT) + Integer.numberOfTrailingZeros(currentBits);
	...
	public void appendBits(int value, int numBits) {
		...
		for (int numBitsLeft = numBits - 1; numBitsLeft >= 0; numBitsLeft--) {
			if ((value & (1 << numBitsLeft)) != 0) {
				bits[(nextSize >>> BITS_PER_UNIT_SHIFT)] |= 1 << (nextSize & BIT_SHIFT_UNIT_LIMIT_MASK);
			}
			nextSize++;
		}
		size = nextSize;
	...
	public void appendBitArray(QuickBitArray other) {
	...
	public void xor(QuickBitArray other) {
		if (size != other.size) {
	...
	public void toBytes(int bitOffset, byte[] array, int offset, int numBytes) {
		for (int i = 0; i < numBytes; i++) {
			...
			for (int j = 0; j < BITS_PER_BYTE; j++) {
				if (get(bitOffset)) {
					theByte |= 1 << (BIT_SHIFT_BYTE_LIMIT_MASK - j);
				}
				bitOffset++;
			}
	...
	public void reverse() {
		...
		if (size != (oldBitsLen << BITS_PER_UNIT_SHIFT)) {
			int leftOffset = (oldBitsLen << BITS_PER_UNIT_SHIFT) - size;
	...
	private static int[] makeArray(int size) {
		return new int[((size + BIT_SHIFT_UNIT_LIMIT_MASK) >>> BITS_PER_UNIT_SHIFT)];
	}
	 ...
	 */
	public QuickBitArray shift(int amount) {
...
//		return this;
	...
	public QuickBitArray shiftRight() {
		QuickBitArray clone = new QuickBitArray(this.getSize());
	...
	public QuickBitArray shiftLeft() {
		QuickBitArray clone = new QuickBitArray(this.getSize());
		if (this.get(0)) {
...
//	
//	private static int[] getCarryMasksForOring(int amount, QuickBitArray qba) {
...
//	}
	...
	private static QuickBitArray randomFill(QuickBitArray qba) {
		Random r = new Random();
	...
	public static QuickBitArray createRandomArrayOfLength(int len) {
		QuickBitArray qba = new QuickBitArray(len);
		return randomFill(qba);
	}

	public static void main(String[] args) {
		QuickBitArray qba = createRandomArrayOfLength(75);
	...
	public boolean equals(Object o) {
		if (!(o instanceof QuickBitArray)) {
			return false;
		}
		QuickBitArray other = (QuickBitArray) o;
	...
	@Override
	public QuickBitArray clone() {
		return new QuickBitArray(bits.clone(), size);
	}
